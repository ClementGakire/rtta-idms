...
<div class="text-center">
  <a href="/bills/create" class="btn btn-info btn-rounded mb-4">Create New Bill Payment</a>
        ...
        <tbody>
            ...
            <td>{{ number_format($bill->quoted_amount) }}</td>
            <td>{{ number_format($bill->amount) }}</td>
            <td>{{ number_format(($bill->quoted_amount)-($bill->amount)) }}</td>
            <td>{{ $bill->payment_date }}</td>
        		@if(Auth::user()->role_id == 1)
        		<td class="text-left pl-4"><a href="/bills/{{$bill->id}}/edit"><i class="fas fa-edit text-success" style="padding-left: 4px;"></i></a><form action="{{ action('BillsController@destroy', [$bill->id]) }}" method="POST">
                                 {{ csrf_field() }}
                                <input type="hidden" name="_method" value="delete">
                                <button onclick="return confirm('Do you want to delete this expense type?')" href="">
